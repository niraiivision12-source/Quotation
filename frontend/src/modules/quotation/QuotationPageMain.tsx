...
export default function QuotationPageMain() {
  ...
  const [quotationType, setQuotationType] = useState<"LEAD" | "CUSTOMER">("LEAD");
  ...
  function handleReset() {
    ...
    setPreviewPayload(null); setIsPreviewOpen(false); setPreviewDetails({});
  }
  ...
  function buildQuotationPayload() {
    if (!phase) { toast.error("Select quotation phase"); return null; }
    if (!billingUserId) { toast.error("Select quotation owner"); return null; }
    if (quotationType === "LEAD" && !leadId) { toast.error("Select a lead"); return null; }
    if (quotationType === "CUSTOMER" && !customerId) { toast.error("Select a customer"); return null; }
