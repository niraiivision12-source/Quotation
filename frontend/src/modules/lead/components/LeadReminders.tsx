...
export default function LeadReminders({ leadId }: Props) {
  ...
  const items = (remindersData?.items ?? []).filter((rem) => {
    if (rem.leadId !== leadId) return false;
          ...
          <DialogContent>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <Input value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1" placeholder="Call lead" required />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground">Description</label>
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 text-xs" placeholder="Discuss requirement details..." />
              </div>
