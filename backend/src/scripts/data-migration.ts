import { PrismaClient, ProductCategory, OpportunityStatus } from '@prisma/client';

const prisma = new PrismaClient();

function mapLeadStatusToOpportunityStatus(status: string): OpportunityStatus {
  switch (status) {
    case 'NEW':
      return OpportunityStatus.NEW;
    case 'CONTACTED':
    case 'NOT_RESPONDING':
      return OpportunityStatus.CONTACTED;
    case 'QUOTATION_SENT':
      return OpportunityStatus.QUOTATION_SENT;
    case 'NEGOTIATION':
      return OpportunityStatus.NEGOTIATION;
    case 'WON':
...
function mapProjectPhaseToProductCategory(phase: string): ProductCategory {
  switch (phase) {
    case 'PIPES':
      return ProductCategory.PIPES;
    case 'WIRING':
      return ProductCategory.WIRES;
    case 'SWITCHES':
      return ProductCategory.SWITCHES;
    case 'LIGHTS':
      return ProductCategory.LIGHTS;
    case 'FANS':
      return ProductCategory.FANS;
    case 'OTHERS':
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const lead of leads) {
      ...
      const opportunity = await tx.opportunity.create({
        data: {
          ...
          category: ProductCategory.OTHERS, // default for general leads
      ...
      console.log(
        `Migrated Lead ${lead.name} -> Opportunity ID ${opportunity.id} (${quotationsCount.count} quotes, ${remindersCount.count} reminders, ${tasksCount.count} tasks updated).`
      );
    }
    ...
    for (const project of projects) {
      for (const tracker of project.phaseTracking) {
        ...
        console.log(
          `Migrated Project ${project.projectName} (${tracker.phase}) -> Opportunity ID ${opportunity.id} (${quotationsCount.count} quotes, ${paymentsCount.count} payments, ${remindersCount.count} reminders, ${tasksCount.count} tasks updated).`
        );
      }
    }
  });
